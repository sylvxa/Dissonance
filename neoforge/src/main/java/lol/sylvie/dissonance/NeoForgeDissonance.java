...
public class NeoForgeDissonance {
    ...
    public NeoForgeDissonance(ModContainer container, IEventBus bus) {
        ...
        bus.addListener(EventPriority.HIGHEST, (Consumer<PlayerEvent.PlayerLoggedInEvent>) event -> {
            if (!(event.getEntity() instanceof ServerPlayer player)) return;
            Component failureReason = DiscordLinking.canPlayerJoin(player.level().getServer(), player.getGameProfile());
            if (failureReason == null) return;
            player.connection.disconnect(failureReason);
        });
    ...
    public void registerEvents(IEventBus bus) {
        ...
        NeoForge.EVENT_BUS.addListener((Consumer<PermissionGatherEvent.Nodes>) event -> {
            DissonancePermissions.BOOLS.forEach((node, value) -> {
                PermissionNode<Boolean> permissionNode = new PermissionNode<>(Constants.MOD_ID, node, PermissionTypes.BOOLEAN, (serverPlayer, uuid, ctx) -> value);
                NeoForgePlatformHelper.BOOLEAN_NODES.put(node, permissionNode);
        ...
        bus.addListener((Consumer<ServerChatEvent>) event -> {
            MinecraftToDiscordBridge.onPlayerChatMessage(event.getPlayer(), event.getRawText());
        });

        bus.addListener((Consumer<PlayerEvent.PlayerLoggedInEvent>) event -> {
            MinecraftToDiscordBridge.onPlayerJoin(event.getEntity());
        });

        bus.addListener((Consumer<PlayerEvent.PlayerLoggedOutEvent>) event -> {
            MinecraftToDiscordBridge.onPlayerLeave(event.getEntity());
        });
    }
}